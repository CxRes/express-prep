...
describe("PREP Middleware Notifications Response without Representation", () => {
  ...
  it("should contain Vary header with Accept-Events and Last-Event-ID", async () => {
    const vary = response.headers.get("vary");
    expect(vary.toLowerCase()).toMatch(/accept-events/);
    expect(vary.toLowerCase()).toMatch(/last-event-id/);
  });
  ...
  it("should output the representation as the first part", async () => {
    ...
    await expect(representation.text()).resolves.toBeFalsy();
  });
